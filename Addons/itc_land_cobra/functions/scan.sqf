...

{
  ...
  _angleTo = abs (((_dir) + 360) - ((_cbr getDir _x) + 360));
  _inRange = _x distance _cbr < 4000;
  if(_angleTo < 23 && _inRange && !(terrainIntersectASL [(getPosASL _x) vectorAdd [0,0,100], (getPosASL _cbr) vectorAdd [0,0,3]])) then {
    _toRemove pushBack _x;
	_impact = [getPos _x, 7];
    [_cbr, _impact] call itc_land_cobra_fnc_processImpact;
  };
...
missionNameSpace setVariable ["itc_land_cobra_modules", _modules - _toRemove];
